...
В трее — кнопка **Logs**: открывает окно с логом текущей сессии (обновляется в
реальном времени, есть Reveal/Clear). Если Connect не подключается — причина
будет там. Файл: `~/Library/Application Support/VKTurnProxy/agent.log`.
