...
import Network
import os.log
// CoreWLAN replaces iOS' NEHotspotNetwork for the SSID diagnostic. It's the
// macOS way to read the current Wi-Fi network name; ssid() returns nil (never
// crashes) when Location Services haven't been authorised, which is fine since
// the SSID is only decoration in the [PathMonitor] log line.
import CoreWLAN
...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private static let sharedDefaultsSuiteName = AppGroup.identifier
    ...
    private func startPathMonitoring() {
        ...
        monitor.pathUpdateHandler = { [weak self] path in
            ...
            // SSID enrichment for wifi paths. On iOS this used the async
            // NEHotspotNetwork.fetchCurrent; on macOS the equivalent is
            // CoreWLAN's CWWiFiClient.interface().ssid(), which is a fast
            // synchronous call. It returns nil when Location Services aren't
            // authorised (macOS gates the SSID behind location, same as iOS
            // gates it behind the wifi-info entitlement) — we proceed in
            // either case so path event logging never blocks on it.
            if path.usesInterfaceType(Network.NWInterface.InterfaceType.wifi) {
                self.currentWiFiSSID = CWWiFiClient.shared().interface()?.ssid()
                process()
            } else {
