...
struct ContentView: View {
    ...
    // macOS-only: the packet tunnel is a system extension that has to be
    // activated (and user-approved) once before the VPN can be configured.
    // On iOS this had no counterpart — the app extension loads automatically.
    ...
    /// Status/CTA banner for the packet-tunnel system extension. Renders
    /// nothing once the extension is active; otherwise it explains the current
    /// state and offers an Install/Enable button. macOS-specific — there is no
    /// iOS equivalent (app extensions load without an install step).
    @ViewBuilder
    private var systemExtensionBanner: some View {
        switch sysext.state {
        case .active:
            EmptyView()
        case .activating:
            HStack(spacing: 8) {
                ...
                Text("Installing tunnel system extension…")
                    .font(.caption)
        ...
        case .needsApproval:
            VStack(spacing: 6) {
                Text("Approve the VK Turn Proxy system extension")
                    .font(.caption).bold()
                Text("Open System Settings ▸ General ▸ Login Items & Extensions ▸ Network Extensions (or Privacy & Security on Ventura) and allow “VK Turn Proxy”, then it will finish enabling.")
                    .font(.caption2)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
                Button("Try again") { sysext.activate() }
                    .font(.caption)
            }
            .padding(.horizontal)
        case .failed(let msg):
            VStack(spacing: 6) {
                Text("Tunnel extension failed to install")
                    .font(.caption).bold()
                    .foregroundColor(.red)
                Text(msg)
                    .font(.caption2)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
                Button("Retry install") { sysext.activate() }
                    .font(.caption)
            }
            .padding(.horizontal)
        case .unknown:
    ...
    var body: some View {
        // NavigationStack (macOS 13+) gives the same single-column push
        // navigation the iOS build had with NavigationView — Logs / Settings
        // push onto the same window rather than opening a sidebar+detail
        // split (which is what a plain NavigationView produces on macOS).
        NavigationStack {
            // ScrollView keeps the whole control panel reachable when the
            // window is short — the stats grid can push Logs/Settings below
            // the fold otherwise.
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    // macOS system-extension status banner. On first launch
                    // (or after a macOS update invalidates the approval) the
                    // packet-tunnel system extension must be installed and
                    // approved once before the VPN can start. Hidden once the
                    // extension is active. iOS had no equivalent step.
                    systemExtensionBanner
                    ...
                    // peerAddress / WG key), OR while the packet-tunnel system
                    // extension isn't active yet (nothing to start until then).
                    // Never disables the Disconnect action.
                    .disabled(tunnel.status != .connected && tunnel.status != .connecting
                              && (configValidationError != nil || !sysext.state.isActive))
            ...
            // Ask the OS to install/activate the packet-tunnel system extension
            // as soon as the UI appears. No-op (returns .completed immediately)
            // once it's already active, so this is safe to run every launch.
            .onAppear { sysext.activate() }
...
struct SettingsView: View {
    ...
    // Backup & Restore state. Export/Import present NSSavePanel/NSOpenPanel
    // imperatively from their button actions (MacFilePanels), so unlike the iOS
    // build there's no exportURL/share-sheet or document-picker state here — the
    // parsed-but-unconfirmed import config plus each confirm alert's `show*`
    // flag are all that remain. Independent flags keep rapid taps from colliding.
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Button(action: {
                    // macOS: pick the backup JSON via NSOpenPanel (the iOS
                    // build used a UIDocumentPicker sheet). Same lenient type
                    // set so files that lost their .json UTI still show.
                    MacFilePanels.importFile(contentTypes: [.json, .text, .data, .item]) { url in
        ...
        // Export (NSSavePanel) and Import (NSOpenPanel) are presented
        // imperatively from their button actions above — the AppKit idiom —
        // instead of the iOS sheet-based UIActivityViewController /
        // UIDocumentPicker. No SwiftUI sheet needed for those flows here.
        //
    ...
    private func handleExport() {
        do {
            ...
            // macOS: save the freshly-written temp backup to a user-chosen
            // location via NSSavePanel (the iOS build handed it to a share
            // sheet). Same file, native destination picker.
            MacFilePanels.exportCopy(of: url, suggestedName: url.lastPathComponent) { msg in
...
struct CaptchaWebView: View {
    ...
    var body: some View {
        ...
        // macOS sheets size to content; WKWebView has no intrinsic size, so pin
        // a usable window size for the captcha flow.
        .frame(minWidth: 460, idealWidth: 480, minHeight: 640, idealHeight: 720)
    }
}

struct CaptchaWKWebView: NSViewRepresentable {
    let url: URL
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // macOS 13.3+ requires explicit opt-in for Safari Web Inspector on
        // WKWebViews. Wrapped in #if DEBUG so Release builds don't expose the
        // WebView to dev tools. Enables: Safari → Develop → this Mac → captcha
        // WebView, then the Network tab shows the real HTTP/2 headers Safari
        // sends to id.vk.ru — needed for matching the Go-side PoW client to
        // the captured Safari fingerprint.
        #if DEBUG
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button(action: {
                    // macOS: save the combined log (archive .1 + current) to a
                    // user-chosen location via NSSavePanel. iOS used a share sheet.
                    if let url = exportShareableLogURL(),
                       FileManager.default.fileExists(atPath: url.path) {
                        MacFilePanels.exportCopy(of: url, suggestedName: "vpn-export.log") { _ in }
                    }
                }) {
...
// LogTextView (NSTextView) lives in PlatformSupport.swift. File export/import
// on macOS uses NSSavePanel/NSOpenPanel via MacFilePanels (same file), so the
// iOS UIActivityViewController / UIDocumentPicker wrappers are gone.
