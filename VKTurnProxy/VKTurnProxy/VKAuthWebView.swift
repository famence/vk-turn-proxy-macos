...
struct VKAuthWebView: View {
    ...
    var body: some View {
        ...
        // macOS sheets size to their content and WKWebView has no intrinsic
        // size, so pin a usable window size for the login flow.
...
/// cookie store for the remixsid + p pair. (WKWebView is an NSView on macOS;
/// on iOS this was a UIViewRepresentable — the coordinator logic is identical.)
struct VKAuthWKWebView: NSViewRepresentable {
