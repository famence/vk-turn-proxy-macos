...
func init() {
	...
	// Soft cap on Go runtime memory footprint.
	//
	// On iOS this defends against the NetworkExtension ~50 MB jetsam
	// limit — the original iOS build ran at a 35 MB GOMEMLIMIT, which
	// forced aggressive GC + eager page return to stay under the
	// per-process ceiling that iOS' jetsam enforces on extensions.
	//
	// macOS has NO such per-process jetsam ceiling for a packet-tunnel
	// extension: the transport machinery below (WireGuard, TURN, DTLS,
	// SRTP, the cred pool, path monitoring) is byte-for-byte identical to
	// iOS, but the memory environment is a desktop, not a phone. Keeping
	// the 35 MB cap here would only manufacture GC pressure — 2-3× more
	// GC cycles and a measurable speedtest regression at 30 conns — for
	// no benefit, since nothing kills the process at 50 MB on the Mac.
	//
	// So we raise the soft cap to a value that comfortably covers the
	// SRTP path's working set (steady-state heap-inuse ~11-14 MB +
	// stacks + runtime + spikes during heap pressure) with headroom,
	// restoring the full-throughput default GC cadence while still
	// bounding runaway growth. The FreeOSMemory ticker below is kept
	// (cheap, and it keeps RSS tidy after traffic bursts).
	debug.SetMemoryLimit(512 << 20)
	log.Printf("bridge: GOMEMLIMIT set to 512 MB (macOS has no NE jetsam ceiling — raised from the iOS 35 MB cap to restore full-throughput GC cadence)")
